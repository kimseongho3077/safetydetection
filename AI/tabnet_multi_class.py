...
# TabNet Feature Importance 추출
tabnet_feature_importance = clf.feature_importances_

# Feature Importance 정리 (내림차순 정렬)
df_tabnet_importance = pd.DataFrame({
    "Feature": df.drop(columns=['Index', '심박', '호흡', '피부온도', '혈중산소농도', '일상', '상태']).columns,  # 피처 이름
    "TabNet Importance": tabnet_feature_importance
}).sort_values(by="TabNet Importance", ascending=False)  # 내림차순 정렬

# 출력
print("TabNet Feature Importance:")
print(df_tabnet_importance)

# CSV 파일로 저장
df_tabnet_importance.to_csv("tabnet_feature_importance.csv", index=False)
